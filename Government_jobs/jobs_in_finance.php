...
<?php include 'display/governmentdisplay.php' ?>
...
<nav class="white">
    <div class="nav-wrapper">
      <form autocomplete="off" id="formbihar">
 ...
 <?php  if(file_exists($filename))
{
  ...
  <div class="card-body">
    ...
    <h5 class="card-title"><?php if($deptpostname[$j]!=''){echo $deptpostname[$j];} else 
    echo $postname[$j]; ?></h5>
    <p class="card-text"><?php echo $mainintro[$j]; ?></p>
   <span><h6>Last Date to apply:</h6><p><b><?php echo $ldtapply[$j]; ?></b></p></span>
    <?php if($brddeptid[$j]!=''){?>
    <a id=<?php echo $brddeptid[$j]?> href="./sarkari_finance_jobs" target="_blank" class="btn btn-primary idex">View Details</a>
    <?php } else { ?>
    <a id=<?php echo $id[$j]?> href="./sarkari_finance_jobs?a=finance_job" target="_blank" class="btn btn-primary idex">View Details</a><?php }?> 
...
<!-- row -->
    ...
    <?php } else {?>
      <?php   // A session is required
        ...
        // Add a few messages
        $msg->warning('No jobs notification related to Noida present but please select from other jobs ');
