...
<?php include 'upscjobdisplay.php' ?>
 ...
 <?php  if(file_exists($filename))
{
  ...
  <div class="card-body">
   ...
   <span><h6>Last Date to apply:</h6><p><b><?php echo $ldtapply[$j]; ?></b></p></span>
    <?php if($brddeptid[$j]!=''){?>
    <a id=<?php echo $brddeptid[$j]?> href="./sarkari_bihar_jobs" target="_blank" class="btn btn-primary idex">View Details</a>
    <?php } else { ?>
    <a id=<?php echo $id[$j]?> href="./sarkari_bihar_jobs?a=bihar_job" target="_blank" class="btn btn-primary idex">View Details</a><?php }?> 
...
<!-- row -->
    ...
    <?php } else {?>
      <?php   // A session is required
        ...
        // Add a few messages
        $msg->warning('No jobs notification related to UPSC present but please select from following jobs ');
